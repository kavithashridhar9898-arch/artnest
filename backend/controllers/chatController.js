    ...
    try {
        ...
        const [conversations] = await db.query(`
            SELECT cc.*,
                   ...
                   CASE 
                       WHEN cc.user1_id = ? THEN u2.full_name
                       ELSE u1.full_name
                   END as other_user_name,
                   CASE 
                       WHEN cc.user1_id = ? THEN u2.profile_pic
                       ELSE u1.profile_pic
                   END as other_user_pic,
                   CASE 
                       WHEN cc.user1_id = ? THEN u2.user_type
                       ELSE u1.user_type
                   END as other_user_type,
                   (SELECT COUNT(*) FROM chat_messages 
                    WHERE conversation_id = cc.id AND receiver_id = ? AND is_read = FALSE) as unread_count
